...
class Student
{
    ...
    public static function loadStudentsFromFile($file)
    {
        $rows = file($file);
    ...
    public static function saveStudentsToTxt(array $students, $file)
    {
        $rows = [];
...
$students = Student::loadStudentsFromFile($file);

foreach ($students as $student) {
    echo $student->getFullName() . ' ' . $student->birthDate . PHP_EOL;
}

Student::saveStudentsToTxt($students, __DIR__ . $file);
