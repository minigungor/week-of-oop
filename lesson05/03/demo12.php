...
class Player
{
    ...
    public function setVolume($volume)
    {
        if(!(0 <= $volume && $volume <= 10)) {
            throw new \OutOfBoundsException('Громкость вне диапонзона');
        }
        $this->volume = $volume;
    }
...
try {
    ...
    echo 'Ошибка плеера: ' . $e->getMessage() . PHP_EOL;
} catch (\LogicException $e) {
    echo 'Ошибка логики: ' . $e->getMessage() . PHP_EOL;
} catch (\Exception $e) {
    echo 'Другая ошибка: ' . $e->getMessage() . PHP_EOL;
}